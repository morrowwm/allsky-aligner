...
function VirtualSky(input){
	...
	this.ra_off = 0;
	this.dc_off = 0;
	// WMM adjust projection
	this.aligner = {};
	this.aligner.theta = 0.0;
	this.aligner.phi = 0.0; //TODO this is unused?
	this.aligner.scale_factor=1.0;
	this.aligner.xoffset = 0.0; 
	this.aligner.yoffset = 0.0; 
	this.aligner.xorigin = 0.0; 
	this.aligner.yorigin = 0.0;
	...
	this.projections = {
		...
		'fisheye':{
			...
			azel2xy: function(az,el,w,h){
				var radius = h/2;
				var rotation = this.aligner.theta * this.d2r;
				var r = this.aligner.scale_factor * radius*Math.sin(((Math.PI/2)-el)/2)/0.70710678;	// the field of view is bigger than 180 degrees
				return {x:(w/2-r*Math.sin(az+rotation)),y:(radius-r*Math.cos(az+rotation)),el:el};
			},
			xy2azel: function(x, y, w, h) {
				var radius = h/2;
...
VirtualSky.prototype.createSky = function(){
	...
	if(this.c && this.c.getContext){
		...
		// https://github.com/AllskyTeam/allsky/issues/4287
...
// console.log("drawImmediate() called by " + whofrom);
	...
	var xcenter = this.aligner.xoffset+this.wide/2;
	var ycenter = this.yoffset+this.tall/2;
	...
	if(this.polartype){
		c.moveTo(xcenter, ycenter);
		c.closePath();
		c.beginPath();
		c.arc(xcenter,ycenter,-0.5+ycenter,0,Math.PI*2,true);
		c.closePath();
		if(!this.transparent){
	...
	if(this.hasGradient()){
		if(!this.skygrad){
			...
			// draw shapes
			if(this.projection.polartype){ c.arc(xcenter,ycenter,ycenter,0,2*Math.PI,false); c.fill(); }
...
VirtualSky.prototype.startClip = function(){
	var xcenter = this.aligner.xoffset+this.wide/2;
	var ycenter = this.aligner.yoffset+this.tall/2;
	
	if(this.polartype){
		this.ctx.save();
		this.ctx.beginPath();
		this.ctx.arc(xcenter,ycenter,-0.5+ycenter,0,Math.PI*2,true);
		this.ctx.clip();
	}
	return this;
};
	...
	for(i = 0; i < this.stars.length; i++){
		if(this.stars[i][1] < this.magnitude){
			...
			if(this.isVisible(p.el) && !isNaN(p.x) && !this.isPointBad(p)){
				...
				d *= f;
				var x = this.aligner.xoffset+p.x;
				var y = this.aligner.yoffset+p.y;
				c.moveTo(x+d,y);
	...
	for(p = 0 ; p < this.planets.length ; p++){
		...
		if(this.planets[p].length == 3){
			...
			if(this.planets[p][2].length%4 == 0){
				var firstEntry = this.planets[p][2][0]; 
				var lastEntry = this.planets[p][2][(this.planets[p][2].length-4)];
				if(this.jd > firstEntry && this.jd < lastEntry){
		...
		if((this.showplanets || this.showplanetlabels) && this.isVisible(pos.el) && mag < this.magnitude && !this.isPointBad(pos)){
			...
			if(d < 1.5) d = 1.5;
			this.drawPlanet(this.aligner.xoffset+pos.x,this.aligner.yoffset+pos.y,d,colour,this.planets[p][0]);
		}
			...
			for(i = 0 ; i < this.planets[p][2].length-4 ; i+=4){
				...
				if(previous.x > 0 && previous.y > 0 && this.isVisible(point.el)){
					...
					if(Math.abs(point.x-previous.x) < maxl){
						c.lineTo(this.aligner.xoffset+point.x,this.aligner.yoffset+point.y);
					}
				}
				previous = point;
			}
	...
	if(this.showplanets || this.showplanetlabels){
		...
		if(this.sun) {
			...
			if(this.isVisible(pos.el) && !this.isPointBad(pos)){
				this.drawPlanet(this.aligner.xoffset+pos.x,this.aligner.yoffset+pos.y,5,this.col.sun,"sun");
		...
		if(this.moon) {
			...
			if(this.isVisible(pos.el) && !this.isPointBad(pos)){
				this.drawPlanet(this.aligner.xoffset+pos.x,this.aligner.yoffset+pos.y,5,this.col.moon,"moon");
	...
	for(var c = 0; c < this.lines.length; c++){
					...
					if(this.isVisible(posa.el) && this.isVisible(posb.el)){
						if(!this.isPointBad(posa) && !this.isPointBad(posb)){
							...
							if(Math.abs(posa.x-posb.x) < maxl && Math.abs(posa.y-posb.y) < maxl){
								x.moveTo(this.aligner.xoffset+posa.x,this.aligner.yoffset+posa.y);
								x.lineTo(this.aligner.xoffset+posb.x,this.aligner.yoffset+posb.y);
							}
						}
					}
				}
		...
		if(this.constellation.labels){
			...
			if(this.isVisible(pos.el)){
				...
				x.fillText(label,this.aligner.xoffset+pos.x+xoff,this.aligner.yoffset+pos.y-fontsize/2);
				x.fill();
			}
		}
	}
			...
			for(i = 0; i <= points.length; i++){
				...
				if(posa && this.isVisible(posa.el) && this.isVisible(posb.el) && points[j][2]){
					if(!this.isPointBad(posa) && !this.isPointBad(posb)){
						// Basic error checking: constellations behind us often have very long lines so we'll zap them
						if(Math.abs(posa.x-posb.x) < maxl && Math.abs(posa.y-posb.y) < maxl){
							this.ctx.moveTo(this.aligner.xoffset+posa.x,this.aligner.yoffset+posa.y);
							this.ctx.lineTo(this.aligner.xoffset+posb.x,this.aligner.yoffset+posb.y);
						}
					}
				}
				posa = posb;
		...
		for(i = 0; i < p.length; i+=2){
			...
			if(i==0) this.ctx.moveTo(pb.x,pb.y);
				...
				if(!isNaN(dx) && !isNaN(dy)){
					...
					this.ctx.lineTo(this.aligner.xoffset+pb.x,this.aligner.yoffset+pb.y);
				}else{
					this.ctx.moveTo(this.aligner.xoffset+pb.x,this.aligner.yoffset+pb.y);
				}
			}
			pa = pb;
		}
		...
		if(this.showers[s]){
			...
			if(this.clock > start && this.clock < end){
				...
				if(this.isVisible(pos.el)){
					...
					c.moveTo(this.aligner.xoffset+pos.x+2,pos.y);
					c.arc(this.aligner.xoffset+pos.x,pos.y,2,0,Math.PI*2,true);
					c.fillText(label,this.aligner.xoffset+pos.x+xoff,this.aligner.yoffset+pos.y-fs/2);
...
function joinpoint(s,type,a,b,old,maxl){
	...
	x = s.aligner.xoffset+pos.x;
	y = s.aligner.yoffset+pos.y;
...
VirtualSky.prototype.updateOrientation = function(aligner, file_datetime){
	//if(typeof input.magnitude==="number") this.magnitude = input.magnitude;
	if(typeof aligner.xorigin==="number") this.aligner.xorigin = aligner.xorigin;
	if(typeof aligner.yorigin==="number") this.aligner.yorigin = aligner.yorigin;
	if(typeof aligner.theta==="number") this.aligner.theta = aligner.theta;
	if(typeof aligner.phi==="number") this.aligner.phi = aligner.phi;
	if(typeof aligner.scale_factor==="number") this.aligner.scale_factor = aligner.scale_factor;
	if(typeof aligner.xoffset==="number") this.aligner.xoffset = aligner.xoffset;
	if(typeof aligner.yoffset==="number") this.aligner.yoffset = aligner.yoffset;

	this.updateClock(file_datetime);
		
	//this.draw("updateProperties");
	this.drawImmediate(this.projection, "updateProperties");
	return this;
};
VirtualSky.prototype.getOrientation = function(){
	return ([this.aligner.theta, this.aligner.phi, this.aligner.scale_factor, this.aligner.xoffset, this.aligner.aligner.yoffset, this.filetime]);
}
